...
Добро пожаловать в мой инженерный журнал! Название `b0-log` — это скрытая пасхалка для тех, кто понимает Computer Science: это путь от тяжелых квадратичных алгоритмов к красивой линейной и логарифмической логике ($O(N^2) \to O(\log N)$).

Здесь я не просто собираю решения задач, а препарирую код под микроскопом, исследуя, как заставить движок V8 и процессор работать на пределе возможностей.
